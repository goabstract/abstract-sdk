...
describe("#list", () => {
  test("api", async () => {
    mockAPI("/organizations/org-id/webhooks", [{ id: "webhook-id" }]);
    const response = await API_CLIENT.webhooks.list({
      organizationId: "org-id"
    });
    expect(response).toEqual([{ id: "webhook-id" }]);
  });
});

describe("#info", () => {
  test("api", async () => {
    mockAPI("/organizations/org-id/webhooks/webhook-id", { id: "webhook-id" });
...
describe("#events", () => {
  test("api", async () => {
    mockAPI("/organizations/org-id/webhooks/events", [{ id: "event-id" }]);
    const response = await API_CLIENT.webhooks.events({
      organizationId: "org-id"
    });
    expect(response).toEqual([{ id: "event-id" }]);
  });
});
...
describe("#ping", () => {
  test("api", async () => {
    mockAPI("/organizations/org-id/webhooks/webhook-id/ping", {}, 200, "post");
...
describe("#deliveries", () => {
  test("api", async () => {
    ...
    expect(response).toEqual([{ id: "delivery-id" }]);
  });
});
