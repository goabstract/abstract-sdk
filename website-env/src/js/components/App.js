...
const makeHref = sentence =>
  sentence !== undefined && sentence.replace('/W/g', '-');
...
const chooseParamType = param => {
  switch (param.type.type) {
    case 'NameExpression':
      return (
        <code>
          <a href={`#${param.type.name}`}>{param.type.name}</a>
        </code>
      );
    case 'UnionType':
      return param.type.elements.map((element, i) =>
...
const createProperGithubLink = ({
  ...
  url
    .split('/')
    .map((u, i) => (i === 6 ? 'documentation-migration' : u))
    .join('/');
...
class App extends Component {
  ...
  componentDidMount() {
    fetch('http://localhost:5000/doc.json')
      .then(res => res.json())
      .then(res => {
        this.setState({
          JsonData: res
    ...
    return (
      <div className="layout">
        ...
        <main>
          ...
          {JsonData.map(
            documentedCode =>
              (documentedCode.kind === 'class' && (
                <section className="Document">
                  <h3
                    id={makeHref(documentedCode.name)}
                  ...
                  {documentedCode.context !== undefined && (
                    <p>
                      GitHub:{' '}
                      <a href={createProperGithubLink(documentedCode)}>
                        {documentedCode.context.github.path}
                      </a>
                    </p>
                  )}
                  <code className="Document__CodeExample">
                    {documentedCode.name.includes('Client')
                      ? `new Client()`
                      : `new Client().${documentedCode.name.toLowerCase()}()`}
                  </code>
                  ...
                  <div className="Document__Members">
                    {documentedCode.members.static.length > 0 &&
                      documentedCode.members.static.map(instances => (
                        <div className="Document__Instances">
                          <h4>{instances.name}</h4>
                          <p>
                            GitHub:{' '}
                            <a href={createProperGithubLink(instances)}>
                              {instances.context.github.path}
                            </a>
                          </p>
                          <p>
                            Member of <em>{instances.memberof}</em>
                          </p>
                          <code>
                            new Client().{instances.memberof.toLowerCase()}().
                            {instances.name}(
                            {instances.params.map((param, i) =>
                              i === instances.params.length - 1
                                ? param.name
                                : `${param.name}, `
                            )}
                            )
                        ...
                        <ul>
                          {documentedCode.sees.map(see => (
                            <li>
                              <a
                                href={`#${makeHref(
                                  see.description.children[0].children[0]
                                    .children[0].value
                                )}`}
                              >
                                {
              ...
              (documentedCode.kind == undefined && (
                <section className="Document">
                  ...
                  {documentedCode.description &&
                    documentedCode.description.children.map(desc =>
                      desc.type === 'html' ? (
                        <p dangerouslySetInnerHTML={{ __html: desc.value }} />
                      ) : (
                        <p className="Document__Description">
                          {desc.children.map(child => chooseChildType(child))}
                        </p>
                  ...
                  <div className="Document__Members">
                    ...
                    <ul>
                      {documentedCode.properties.length > 0 &&
                        documentedCode.properties.map((prop, i) => (
                          <li>
                            <span>{prop.name}</span> :{' '}
                            {chooseParamType(prop.type)}
                          </li>
                        ))}
                    </ul>
                  </div>
